...
Asaas billing operations use `Billing__Asaas__ApiKey` from a secret store. Development and Testing are pinned to `https://api-sandbox.asaas.com/v3/`; they reject a production API URL and never send billing requests to the live host. A Sandbox API key is optional for starting the API locally, but required to execute real Sandbox gateway calls. Non-local environments require the production API key and use `https://api.asaas.com/v3/`. Never commit either key. Subscription creation uses a stable local subscription identifier as Asaas `externalReference`, reconciles ambiguous outcomes through Asaas before retrying, and uses a 60-second request timeout; an unknown outcome must be reconciled rather than blindly resubmitted.
