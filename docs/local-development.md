...
Expired subscription grace periods are checked by a background worker in the API and scheduled for midnight in `America/Sao_Paulo` (03:00 UTC). Keep the Railway API service running continuously and configure it to restart on failure; do not attach a Railway Cron schedule to this long-running API service. The worker retries failed runs after five minutes.
